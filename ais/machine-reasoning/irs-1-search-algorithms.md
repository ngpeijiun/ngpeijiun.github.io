...
1.  Human Intelligence
    ...
    Intelligence measures a human or AI's ability to achieve goals across a wide range of environments (*Legg & Hutter, 2007*).

    -   A key aspect of human intelligence is **problem-solving** using **knowledge**, which involves:
        -   Using existing knowledge to solve new problems (**Adaptation by Analogy**)
        -   Learning new knowledge from experiences (**Building Intuition**)
        -   Discovering new knowledge (**Step-by-Step Reasoning**)
    -   Machine intelligence is expected to follow the same principles, leveraging **search**, **reasoning**, **representation**, and **learning**.

2.  What is Artificial Intelligence (AI)?

    AI is machine intelligence that:

    -   Is materialised through computer/software systems.
    -   Is designed to perform **problem-solving** and **decision-making** tasks that require intelligence when done by humans.

3.  How to Evaluate Machine Intelligence

    Human intelligence has traditionally been the gold standard for measuring machine intelligence.

    -   Some AI systems have now demonstrated intelligence exceeding human capabilities in specific domains.
    -   The Turing Test (*Alan Turing, 1950*) is a classical evaluation method:
        -   A hidden computer (A) and a human (B) both converse with a judge (C).
        -   If the judge cannot distinguish between the computer and the human, the machine is considered capable of thinking.

4.  The Significance of the Turing Test

    To pass the Turing Test, a system must demonstrate:

    -   **Natural language understanding & generation** (e.g., LLMs like ChatGPT)
    -   **Knowledge representation**
    -   **Self-learning**
    -   **Automated reasoning**
    -   AI systems like AlphaGo vs. Lee Sedol (2016) showcased machine intelligence surpassing human capabilities in complex decision-making environments.
...
7.  Cornerstones for machine reasoning system
    ...
    -   reasoning (inference for solutions)
    -   learning (generalisation)
    -   searching (searching/inference for solutions or searching for generalisation)
...
4.  Uniform-cost search is optimal

    -   BFS expands nodes level by level, ensuring that the shortest path is found in an unweighted graph. In contrast, uniform-cost search expands nodes with the lowest cumulative cost at each step, ensuring that the shortest path is found in a weighted graph.
    -   When all edge weights are equal, uniform-cost search behaves identically to BFS.

## Informed Search

1.  Combinatorial explosion

    A small increase in the number of basic elements causes an exponential growth in the size of the search space, making the problem significantly more complex.
...
3.  Heuristic function $h(n)$
    ...
    -   Estimates the cost of the shortest path from node $n$ to the goal node.
    -   Measures the difference between the current state or solution and the goal state.
...
5.  Greedy best-first search (GBFS) aims to minimise the estimated cost from current node to the goal node by expanding the node with the smallest $f(n)$ at each step. It maintains a search tree to track all expanded nodes. However, GBFS does not guarantee a globally optimal solution, as it may choose suboptimal (locally optimal) paths that are not the shortest.

    Hill Climbing is a more restrictive local search method and can be considered a variant of GBFS. Unlike GBFS, Hill Climbing does not maintain a search tree for backtracking. Instead, it focuses solely on moving to the next node with the smallest $f(n)$, discarding all other nodes at the same level.

    In both GBFS and Hill Climbing, the evaluation function depends solely on the heuristic function:
...
6.  Improving local search
    ...
    Local search methods, such as Hill Climbing, do not maintain a search tree and only move to better neighbouring nodes. They lack explicit mechanism to prevent revisiting previously explored solutions. While Hill Climbing implicitly avoids bad moves by always "climbing uphill" (choosing better neighbours), it can still encounter challenges such as getting stuck in local maxima, plateaus, or ridges, and may cycle through nearby suboptimal solutions.

    Tabu search improves local search by temporarily forbidding certain moves that could lead back to previously visited solutions, helping to prevent cycling or getting stuck in local optima. It maintains a data structure called a Tabu list, which records forbidden moves or recently visited solutions. During each iteration, the algorithm selects a feasible move, performs it, and adds it to the Tabu list. The Tabu restrictions are typically time-based, meaning moves are removed from the list and become feasible again after a certain number of iterations.

    Simulated annealing enhances local search by combining Hill Climbing with a random walk. Instead of always selecting the best move, it allows for random moves:

    -   If a move improves the current solution, it is always accepted.
    -   If a move does not improve the solution, it may still be accepted with a probability $P$, determined by a temperature parameter $T$.

    The mechanism helps the algorithm escape local optima, plateaus, and ridges by allowing "downhill" moves (non-improving steps) early in the search. Over time, as the temperature decreases, the algorithm becomes less likely to accept worse solutions, focusing on convergence.

7.  A* search is a widely known form of best-first search. It combines uniform-cost search, which expands the node with the lowest cumulative cost $g(n)$, with GBFS, which expands the node with the smallest heuristic estimate $h(n)$. A* is more efficient than uniform-cost search because it focuses on promising paths based on $h(n)$, bypassing irrelevant areas if $h(n)$ is accurate. It is also more optimal than GBFS, as it guarantees finding the shortest path when $h(n)$ is admissible. This is achieved by considering the cumulative cost $g(n)$, ensuring global optimality.
    ...
    -   $g(n)$: The actual cost of the shortest path from the start node to node $n$.
    -   $h(n)$: The estimated cost of the shortest path from node $n$ to the goal node.
